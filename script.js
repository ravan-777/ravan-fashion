...
const img=p=>p.image_url||`assets/${fallbackProducts.some(x=>x.id===String(p.id))?p.id:'1'}.svg`;

function mapProduct(p){
return {...p,id:String(p.id),cat:String(p.category||p.cat||'unisex').toLowerCase(),price:Number(p.price||0),old:Number(p.old_price||p.old||0),tag:p.tag||'',sizes:Array.isArray(p.sizes)&&p.sizes.length?p.sizes:['OS'],colors:Array.isArray(p.colors)&&p.colors.length?p.colors:['Black'],desc:p.description||p.desc||'',new:Number(p.featured||p.new||0),sale:String(p.tag||'').toUpperCase()==='SALE',image_url:p.image_url||'',product_url:p.product_url||'',affiliate_url:p.affiliate_url||'',stock:Number(p.stock||0)}
}
...
if(r.ok&&d.ok&&Array.isArray(d.products))products=d.products.map(mapProduct);
else throw Error('API error');
}catch(e){
console.warn('D1 products unavailable; using fallback catalogue.',e);
...
document.getElementById('cc').textContent=cart.reduce((a,x)=>a+x.qty,0);
document.getElementById('wc').textContent=wish.length
}

function setFilter(f){
filter=f;
document.querySelectorAll('.chips button').forEach(b=>b.classList.toggle('active',b.dataset.f==f));
render();
...
function card(p){
return `<article class="card" onclick="openProduct('${p.id}')"><div class="pic"><img src="${img(p)}" alt="${p.name}">${p.tag?`<span class="badge">${p.tag}</span>`:''}<button class="heart ${wish.includes(p.id)?'saved':''}" onclick="event.stopPropagation();toggleWish('${p.id}')">${wish.includes(p.id)?'♥':'♡'}</button></div><div class="info"><div><h3>${p.name}</h3><p>${p.cat.toUpperCase()}</p></div><div class="price">${p.old?`<span class="old">${money(p.old)}</span>`:''}${money(p.price)}</div></div></article>`
}

function render(){
let q=(document.getElementById('q')?.value||'').toLowerCase();
let a=products.filter(p=>(filter==='all'||p.cat===filter||(filter==='new'&&p.new)||(filter==='sale'&&p.sale)||(filter==='men'&&p.cat==='unisex')||(filter==='women'&&p.cat==='unisex'))&&(p.name+p.desc).toLowerCase().includes(q));
...
document.getElementById('grid').innerHTML=a.map(card).join('')||'<p>No products found.</p>';
if(document.getElementById('search').classList.contains('open'))document.getElementById('results').innerHTML='<div class="grid">'+a.map(card).join('')+'</div>'
}
...
document.getElementById('detail').innerHTML=`<div class="detail"><div class="detailimg"><img src="${img(cur)}" alt="${cur.name}"></div><div class="copy"><small>${cur.cat.toUpperCase()} / RAVAN</small><h2>${cur.name}</h2><div>${cur.old?`<span class="old">${money(cur.old)}</span>`:''}${money(cur.price)}</div><p>${cur.desc}</p><small>SIZE</small><div class="opts" id="sizes">${cur.sizes.map(x=>`<button class="${x==selSize?'sel':''}" onclick="pickSize('${x}')">${x}</button>`).join('')}</div><small>COLOUR</small><div class="opts" id="colors">${cur.colors.map(x=>`<button class="${x==selColor?'sel':''}" onclick="pickColor('${x}')">${x}</button>`).join('')}</div><button class="btn full" onclick="add()">ADD TO BAG — ${money(cur.price)}</button></div></div>`;
document.getElementById('modal').classList.add('open')
}

function pickSize(x){
selSize=x;
document.querySelectorAll('#sizes button').forEach(b=>b.classList.toggle('sel',b.textContent===x))
}

function pickColor(x){
selColor=x;
document.querySelectorAll('#colors button').forEach(b=>b.classList.toggle('sel',b.textContent===x))
}

function add(){
if(!cur)return;
let x=cart.find(x=>x.id===cur.id&&x.size===selSize&&x.color===selColor);
x?x.qty++:cart.push({id:cur.id,size:selSize,color:selColor,qty:1});
save();
closeM();
openCart()
}
...
let h='',t=0;
cart.forEach((x,i)=>{
let p=products.find(p=>p.id===String(x.id));
if(!p)return;
t+=p.price*x.qty;
h+=`<div class="row"><img src="${img(p)}"><div><h4>${p.name}</h4><p>${x.size} / ${x.color}</p><div class="qty"><button onclick="qty(${i},-1)">−</button> ${x.qty} <button onclick="qty(${i},1)">+</button></div></div><div>${money(p.price*x.qty)}<br><button class="remove" onclick="del(${i})">REMOVE</button></div></div>`
});
...
function qty(i,d){cart[i].qty=Math.max(1,cart[i].qty+d);save();openCart()}
function del(i){cart.splice(i,1);save();openCart()}

function openWish(){
document.getElementById('wishItems').innerHTML=wish.map(id=>{
let p=products.find(p=>p.id===String(id));
if(!p)return '';
return `<div class="row"><img src="${img(p)}"><div><h4>${p.name}</h4><p>${money(p.price)}</p></div><button class="remove" onclick="toggleWish('${id}');openWish()">REMOVE</button></div>`
...
const subtotal=cartTotal(),shipping=subtotal>=2999?0:199;
document.getElementById('checkoutItems').innerHTML=cart.map(x=>{
const p=products.find(p=>p.id===String(x.id));
return p?`<div class="checkout-item"><span>${p.name}<small>${x.size} / ${x.color} × ${x.qty}</small></span><b>${money(p.price*x.qty)}</b></div>`:''
}).join('');
...
if(res.status!==503)throw Error(data.error||'Order could not be created.')
}catch(err){
